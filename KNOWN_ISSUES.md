...
### #13. Legacy `mirrorPaymentsOnStateWrite` CF writes v1-schema docs (cross-building collision risk) ✅ RESOLVED 2026-06-01
**Severity**: 🟡 → resolved.
**Issue (historical)**: trigger at `functions/index.js:8324` (added commit `042885f`) wrote payment docs with v1-style ID `${unitId}__${ym}` + delete-by-diff loop. Both anti-patterns from SCALING_PLAN_v2.md §0 (rules 1 + 2 — the mass-delete + cross-building leak mechanisms). Firing under `syncV2=true`, it silently accumulated v1 orphans (7 by 2026-06-01).
**Fix (commit `ec48efa`)**: hard-disabled via early `return;` at top of handler body. Export stays so the Firebase deploy contract is preserved, but the function body is unreachable. The in-handler server mirrors from commit `ba68a4d` cover every server-side payment write correctly (building-aware keys + `_schema:'v2'` + delete-by-event only) — so this legacy trigger is strictly redundant.
**Cleanup**: operator ran `sfaCleanV1OrphanPayments({apply: true})` 2026-06-01 — deleted the 7 accumulated orphans (336__2026-05, 346__2026-05, 408__2026-04, 413__2026-04, 429__2025-11, 433__2026-04, 445__2025-11). Confirmed via `runReconcilePaymentsV2Now` callable: `v1OrphanCount: 0`.
