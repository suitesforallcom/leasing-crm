...
### 80. Renewal/amendment envelopes: одностраничный Amendment вместо второго полного договора (lease/legal, 2026-09-02)

**Problem (юридический):** «Generate & e-sign» для kind='renewal'/'amendment' отправлял ПОЛНЫЙ новый Office Services Agreement (send-путь резолвил hardcoded kind='lease'). Итог: два параллельно живых контракта без supersession-связки, свежая обязанность заплатить второй депозит (IV.3 «Client shall pay the Deposit» без carry-over языка), кривые auto-renewal термы (initial Term нового дока ≠ терм оригинала). Разбор подписанного оригинала 346-го: договор и так авто-продлевается (III.2); документ нужен только при изменении условий — и это amendment, не новый договор.

**Fix (текст согласован Tony 2026-09-02, прецедент M2M):** новый генератор `_dsBuildAmendmentHtml` — одностраничный «Amendment to Office Services Agreement»: recitals со ссылкой на оригинал (dated as of `{{lease_start}}` = u.leaseStart, Entry 78), Term extended through `{{lease_end}}` (commences `{{renewal_start}}` — новое merge-поле: pend.effDate → фолбэк u.until+1), Base Fee, **Deposit carry-over («no additional deposit is due»)**, Automatic Renewal III.2 continues, No Other Changes + «this Amendment controls», Counterparts/E-sign; подписной блок как у договора (Ann Noel pre-signed + /signHere//dateSigned/ якоря). Секции условные (amendment без новой даты конца опускает Term/Auto-Renewal пункты), в placeholder-режиме — все. Wiring: `_resolveLeaseTemplate` получил generated-ветку для renewal/amendment ПЕРЕД stub-фолбэком (пер-юнитовый override и workspace-шаблон выше по каскаду выигрывают — amendment 402-го живёт); `docusignSendEnvelope({docKind})` резолвит своим kind (имя документа 'Lease amendment'); `_slBuildPayloadFromForm` кладёт docKind из pending + renewal_start; `_slDoSend` прокидывает; превью (`_slOpenLeasePreview` → previewKind, `_slRenderPreviewBody` → payload.docKind) показывают ровно тот документ, что уйдёт.

**Invariant:** send-путь и оба превью используют ОДИН docKind; renewal/amendment конверт НЕ содержит второй полный договор (никаких Schedule A / House Rules в Amendment-документе); Amendment обязан нести supersession-строку («this Amendment controls») и deposit carry-over. `_dsBuildInlineLeaseHtml` при любых правках рядом — байт-в-байт (SHA-256).

**Verification:** байт-идентичность полного договора до/после: live `d02ad917…`, raw `c2f1991a…` — совпали. Amendment на фикстуре 346: source='generated-amendment', 12/12 проверок (recital Mar 15 2026, конец Mar 23 2027, старт Sep 16 2026, депозит carry-over, без Schedule A/House Rules, якоря, подпись); фолбэк renewal_start без override → until+1; raw-режим — 10 токенов на месте; скриншот рендера. Parse 4/0, гейты 77-80 зелёные.
