...
- 2026-06-01: ✅ **Stage 1.5 + Stage 2 ACTIVATED** (Tony 2026-06-01: «сделай все по порядку»).
  - `ec48efa` HARD-DISABLED `mirrorPaymentsOnStateWrite` (KNOWN_ISSUES #13 closure). Early `return;` at top of handler body; export preserved для Firebase contract, function body unreachable. In-handler mirrors из `ba68a4d` покрывают все server-side payment writes корректно (building-aware + `_schema:'v2'` + delete-by-event only) — legacy trigger строго redundant.
  - Operator cleanup 2026-06-01: `sfaCleanV1OrphanPayments({apply: true})` удалил 7 накопленных v1 сирот (336__2026-05, 346__2026-05, 408__2026-04, 413__2026-04, 429__2025-11, 433__2026-04, 445__2025-11). Подтверждено через `runReconcilePaymentsV2Now` callable: `v1OrphanCount: 0`.
  - **Phase 1.2 read-switch FLIPPED ON прод**: `sfaTestReadSwitchV2(true)` → `state.settings.syncV2Read = true` (saved), `onSnapshot('workspaces/default/payments')` listener attached. Реал-тайм обновляет `u.payments[ym]` из коллекции (debounced 100ms re-render). Smoke pre-flight `sfaReadSwitchSmoke()` показал «✓ Read-switch SAFE: reconcile clean». Monolith ещё держит payments — strip (Phase 1.3) отложен до 48ч clean soak. Rollback в любой момент `sfaTestReadSwitchV2(false)`.
  - **NEXT** (per runbook Stage 3-onwards): observe ~24-48ч clean reconcile → Stage 3 (Phase 2 dual-write activation `settings.syncBuildingsV2 = true` + `sfaMirrorBuildingsV2()`). После Stage 3 soak → Stage 4 read-switch buildings → Stage 5+6 strip (unlimited buildings goal).
- 2026-06-01 (next morning): ✅ **Stage 3 + Stage 4 БОТА АКТИВИРОВАНЫ** Phase 2 dual-write + read-switch.
  - Bug surface 1: `_buildingForV2` не уплощал `u.points` nested arrays — Firestore reject. Fix `4230485`: применил тот же `pointsFlat`-flattening pattern что fbSanitizeState (lines ~33667-33697) использует для монолит-push'а. Плюс `_buildingFromV2Restore` для reverse при чтении из коллекции (применяется в `_v2BuildingsAttachListener` перед merge'ом в state.buildings).
  - Bug surface 2: `sfaReconcileBuildingsV2` direct getDocs падал с permission denied — `workspaces/{ws}/buildings/` collection не имела client-read rule. Переведено через CF callable `runReconcileBuildingsV2Now` (admin SDK bypass) тем же паттерном что `sfaScalingReconcileHistory` (commit f6926ad).
  - Bug surface 3 (real blocker): client-side WRITES via `_mirrorBuildingsToV2` также падали с permission denied — нет write rule для `/buildings/`. Tony подтвердил «да на rules» (paste в console + reasonable inference). Commit `a738ee0`: добавлены `match /buildings/{bid} {allow read isMember, allow write isEditor}` + `match /scaling/{doc} {allow read isMember, allow write false}` в firestore.rules. Deployed `firebase deploy --only firestore:rules` ✓.
  - **Stage 3 SUCCESS**: `sfaMirrorBuildingsV2()` write all 4 buildings, no permission errors. `sfaReconcileBuildingsV2()` через CF: `state buildings: 4 | v2-docs: 4 | missing 0 / extra 0 / mismatched 0 | ✓ ЧИСТО`.
  - **Stage 4 FLIPPED ON прод**: `sfaTestBuildingsReadSwitchV2(true)` → `state.settings.syncBuildingsRead = true`, `onSnapshot('workspaces/default/buildings')` listener attached. Smoke pre-flight passed. Real-time мерджит building updates в state.buildings с preservation u.payments через PB1.2 listener (debounced 150ms re-render через renderBuildingSelector + _ensureValidUiBuilding + renderUnits + render of active view).
  - **СОСТОЯНИЕ**: 4 layer scaling V2 (Phase 1 dual-write + Phase 1.2 read + Phase 2 dual-write + Phase 2.4 read) **ВСЕ АКТИВНЫ**. Монолит ещё держит state для безопасности. Quota localStorage 4225 KB (87%) растёт из-за дублирования. Strip (Phase 1.3 + 2.5) откроет unlimited buildings — отдельная сессия (HIGH-RISK, past v1 incident).
  - **SOAK**: периодически `sfaScalingReconcileHistory(24)` + `sfaScalingReconcileHistory(24, 'buildings')`. 24-48ч clean подряд → готов к Stage 5 strip.
