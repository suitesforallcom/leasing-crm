...
/* Во сколько раз шире рисуется план в увеличенном виде. Держим рядом с CSS
   (.plan-wrap.zoom .plan-svg { width: … }) — расхождение сделает порог
   читаемости неверным. */
const ZOOM_FACTOR = 3;
...
function planSvg(ctx, b, f, ym, zoom) {
  ...
  const renderPx = zoom ? 360 * ZOOM_FACTOR : 360;
  const scale = renderPx / vw;
  /* ЕДИНЫЙ кегль на весь этаж — как на десктопе. Раньше он считался от
     габаритов каждой фигуры, и получалось два дефекта сразу (оба нашёл
     оператор): номера «ехали» — 101 крупный, 114 мелкий, читалось как каша;
     а мелкие сьюты не подписывались вовсе, потому что их кегль не дотягивал
     до порога читаемости — на правой половине этажа Bay Vista не было НИ
     ОДНОГО номера. Теперь размер один для всех и подпись рисуется всегда:
     где тесно, оператор увеличит. */
  const LABEL_FS = vw / 46;
